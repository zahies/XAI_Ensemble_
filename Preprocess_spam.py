...
def read_and_preprocess_spam():

    if (os.path.isfile("data_spam/spam_after_preprocess.csv") == False):
        to_fit = pd.read_csv("data_spam/Spam.csv", low_memory=False)
        # df = df.drop("Unnamed: 0", axis=1)
        # df.columns = df.columns.str.replace(' ', '_')

        to_fit.rename(columns={"char_freq_!": "char_freq_exclamation", "char_freq_(": "char_freq_parenthesis", "char_freq_$": "char_freq_dollar",
                               "char_freq_;": "char_freq_dot&comm", "char_freq_[": "char_freq_square_parenthesis"})

        print(to_fit.shape)
        print(to_fit.isnull().sum(axis=0))
        print(to_fit.info())
        print(to_fit.nunique().sort_values())
        ...
        print(to_fit.info())
        to_fit.to_csv("data_spam/spam_after_preprocess.csv")
    else:
        to_fit = pd.read_csv("data_spam/spam_after_preprocess.csv")
